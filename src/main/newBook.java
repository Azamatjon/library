...
public class newBook {
	 ...
	 */
	public newBook(String frameName) {
	...
	private void initialize() {
		...
		JLabel logo = new JLabel("Adding a new Book");
		...
		lg20.setSize(600, 40);
		JLabel depr_ = new JLabel("Department:");
		...
		ResultSet departments = db.getAll("Select * from `departments`");
		try {
			dpr_.addItem(new comboItem(" ", -1));
			while (departments.next()) {
				dpr_.addItem(new comboItem(departments.getString("name"), departments.getInt("id")));
			}
		} catch (SQLException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		...
		customButton addNow = new customButton("Add", 100, 40, "images/plus.png");
		addNow.setPosition(100, 20);
		
		
		JLayeredPane lg12 = new JLayeredPane();
		lg12.setSize(600, 90);
		lg12.add(addNow.getButton(), new Integer(2));
		
		
		customButton removeNow = new customButton("Remove", 120, 40, "images/remove.png", new Color(255, 0, 0, 200), new Color(255, 255, 255));
		removeNow.setPosition(230, 20);
		
	
		lg12.add(removeNow.getButton(), new Integer(2));
		
		for (Component btn : addNow.getButton().getComponents()) {
			btn.addMouseListener(new MouseAdapter() {
				...
				public void mouseReleased(MouseEvent e) {
					...
					if (titleField.getText().length() > 0) {
						if (nwbk.add(titleField.getText(), auNameField.getText(), surnauTf.getText(), Integer.parseInt(prct_.getValue().toString()), brct_.getText(), ((int)((comboItem)subj_.getSelectedItem()).getId() != -1)?(int)((comboItem)subj_.getSelectedItem()).getId():null, Integer.parseInt(columnLoc.getValue().toString()), Integer.parseInt(rowLoc.getValue().toString()), Integer.parseInt(bkn__.getValue().toString()), pubName.getText(), ((int)((comboItem)catge_.getSelectedItem()).getId() != -1)?(int)((comboItem)catge_.getSelectedItem()).getId():null, descr__.getText(), yert_.getText(), ((int)((comboItem)dpr_.getSelectedItem()).getId() != -1)?(int)((comboItem)dpr_.getSelectedItem()).getId():null)){
							showMessage("Success, the book was added successfully!", true);
						}						
